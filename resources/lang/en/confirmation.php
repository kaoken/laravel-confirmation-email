<?php
return [
    /*
    |--------------------------------------------------------------------------
    | Confirmation Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines contain the default error messages used by
    | the confirmation class. Please add or edit these messages to suit the application you develop.
    |
    */
    'email_confirmation_subject' => 'Email verification',
    'email_registration_subject' => 'Registration has ended.',
    'msg_first_registration' => 'The first registration has ended.',
    'msg_second_registration' => 'Registration has ended.',
    'msg_move_url' => 'Please click the following address to complete registration.',
    'msg_user_delete' => 'It will be deleted after 24 hours. Please re-register if expiration date expires.',
    'msg_note' => 'Please note that replying to this mail can not be received. For inquiries, please contact from the "inquiry" on the Hoge site.'
];